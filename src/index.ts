...
app.get('/api/posts', async (req, res) => {
  const { userEmail, sortBy } = req.query;

  let orderByCondition: any = { createdAt: 'desc' };
  if (sortBy === 'popular') {
    orderByCondition = { votes: 'desc' };
  }
...
// CREATE POST (Fixed Quote Typo Here)
...
app.patch('/api/posts/:id/vote', async (req, res) => {
  ...
  try {
    ...
    const uId = user.id;
    const targetType = voteType.toUpperCase();
    ...
    if (existingVote) {
      if (existingVote.type === targetType) {
        ...
        voteChange = targetType === "UP" ? 2 : -2;
      }
    } else {
      await prisma.vote.create({
        data: { type: targetType, userId: uId, postId: pId }
      });
      voteChange = targetType === "UP" ? 1 : -1;
    }
...
app.post('/api/users/register', async (req, res) => {
  ...
  try {
    ...
    const user = await prisma.user.create({
      data: { email: email.toLowerCase(), username, password: hashedPassword }
    });
...
// AUTHENTICATION: LOGIN (Mapped to /api/auth/login to fix frontend compatibility!)
...
app.post('/api/communities/:name/join', async (req, res) => {
  ...
  try {
    const updatedCommunity = await prisma.community.update({
      where: { name: name.toLowerCase() },
      data: { members: { connect: { email: userEmail } } },
...
app.post('/api/communities/:name/leave', async (req, res) => {
  ...
  try {
    const updatedCommunity = await prisma.community.update({
      where: { name: name.toLowerCase() },
      data: { members: { disconnect: { email: userEmail } } },
      include: { _count: { select: { members: true } } }
    });
